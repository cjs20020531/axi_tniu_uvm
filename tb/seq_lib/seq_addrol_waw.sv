...
class seq_addrol_waw extends rknp_base_seq;
  ...
  task body();
    ...
    if (num_txn < 2)
      `uvm_fatal("SEQ_ADDROL_WAW", "num_txn must be at least 2")
    ...
    for (int unsigned i = 0; i < num_txn; i++) begin
      ...
      if (i == 0) begin
        if (!it.randomize() with {
          ...
          `uvm_fatal("SEQ_ADDROL_WAW", "First request randomization failed")
        ...
        if (!it.randomize() with {
          ...
          `uvm_fatal("SEQ_ADDROL_WAW", "Last request randomization failed")
      end
      else begin
        if (!it.randomize() with {
              status     == axi_tniu_protocol_pkg::ST_OK;
          ...
          `uvm_fatal("SEQ_ADDROL_WAW", "Middle request randomization failed")
      end
