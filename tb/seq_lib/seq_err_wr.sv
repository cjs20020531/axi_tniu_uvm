`ifndef SEQ_ERR_WR_SV
`define SEQ_ERR_WR_SV

class seq_err_wr extends rknp_base_seq;
  `uvm_object_utils(seq_err_wr)

  function new(string name = "seq_err_wr");
    super.new(name);
    num_txn = 4;
  endfunction
  ...
  task body();
    for (int unsigned i = 0; i < num_txn; i++) begin
      ...
      if (!it.randomize() with {
        ...
        `uvm_fatal("SEQ_ERR_WR", "Randomization failed")
      complete_item(it, "SEQ_ERR_WR");
    end
  endtask
endclass : seq_err_wr
