`ifndef SEQ_ERR_WRW_SV
`define SEQ_ERR_WRW_SV

class seq_err_wrw extends rknp_base_seq;
  `uvm_object_utils(seq_err_wrw)

  function new(string name = "seq_err_wrw");
    super.new(name);
  endfunction
  ...
  task body();
    for (int unsigned i = 0; i < num_txn; i++) begin
      ...
      if (!it.randomize() with {
        ...
        `uvm_fatal("SEQ_ERR_WRW", "Randomization failed")
      complete_item(it, "SEQ_ERR_WRW");
    end
  endtask
endclass : seq_err_wrw

`endif // SEQ_ERR_WRW_SV
